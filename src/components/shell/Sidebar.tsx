...
export function Sidebar() {
  const {
    ...
    mobileMenuOpen, setMobileMenuOpen, activeWorkspace,
  } = useWorkspaceStore();
  const { profile } = useAuthStore();
  const navigate = useNavigate();
  const location = useLocation();

  // Auto-collapse on medium screens, close mobile menu on route change
  useEffect(() => {
  ...
  return (
    <>
      {/* Mobile hamburger button (visible < 768px, rendered by AppShell) */}
      ...
      >
        ...
        <button
          onClick={() => setMobileMenuOpen(false)}
          className="absolute top-3 right-3 w-7 h-7 rounded-full bg-slate-700 flex items-center justify-center text-slate-300 hover:bg-slate-600 md:hidden"
        >
          <X className="w-4 h-4" />
        </button>
                ...
                {section.items.map(({ to, icon: Icon, label }) => (
                  <li key={to}>
                    ...
                    >
                      ...
                      {(!sidebarCollapsed || mobileMenuOpen) && <span>{label}</span>}
                    </NavLink>
                  </li>
                ))}
              </ul>
        ...
        <button
          ...
          className="absolute -right-3 top-16 w-6 h-6 rounded-full bg-slate-700 border border-slate-600 flex items-center justify-center text-slate-300 hover:bg-slate-600 hover:text-white transition-colors z-50 hidden md:flex"
